...
# ---- 确认层 C：杠杆挤满（顶部用，与底部的「估值投降」严格对称）----
# 融资余额的 250 日分位。分位高 = 杠杆挤满 = 真狂热。
#
# 实测（_explore/d72、d73）：
#   顶部信号（首破 100）不加任何过滤时，长历史 trail 上 T+60 下跌 15/29 = 51.7%，
#   随机基线 47.5%，**p = 0.39 —— 完全不显著**，也就是说顶部信号本来就是无效的。
#   加上「融资余额分位 ≥70」之后：
#     长历史 trail  T+60 下跌 8/8 = 100%（基线 47.6%，p = 0.0027）
#     长历史 expand T+60 下跌 7/7 = 100%（基线 46.7%，p = 0.0048）
#   门槛 50 / 60 / 65 / 70 / 75 / 80 / 90 / 95 结果完全一致（平台，不是尖峰）。
#
#   **这也解释了为什么近 3 年顶部只有 2/9**：近 3 年 9 次破 100，
#   融资余额分位全部落在 10~18（2024-09 那波是政策刺激的急涨，杠杆还没加回来），
#   一次都没有「挤满」过。而 2015-05、2020-07 那两次真正的顶，分位接近 100。
#   → **近 3 年根本没出现过真正的杠杆顶。**
#
# 尺度提醒：顶部信号的自然尺度是 **T+60**，不是 T+20。
#   杠杆挤满的信号 T+20 下跌只有 3/7，T+60 是 7/7，均收益 +0.50% → −5.32%。
#   而且即使方向对，中途最大踏空仍有 +15% —— **顶部信号不能用来做空/清仓**。
MG_PCT_MIN = 70.0


def margin_series() -> pd.Series | None:
    from . import data as D
    return D.load_margin_pct()
...
def grade(e: dict) -> str | None:
    ...
    实测（T+60 盈利 / 下跌）：
      ...
      顶部 A 级：长历史 T+60 下跌 8/8（但样本只有 8，且中途最大踏空 +15%）
      顶部 C 级：长历史 T+60 下跌 15/29 = 51.7%，与随机基线 47.5% 无显著差异
    ...
    if e.get("kind") == "top":
        ...
        if fr is None:
            return None
        return "A" if fr else "C"
    return None


def events(p: pd.DataFrame, reso: pd.Series | None = None, bna: pd.Series | None = None,
           mg: pd.Series | None = None):
    """实时可执行信号（逐日模拟，只用当日及历史数据，无未来函数）。

    底部  昨日分值 ≤ ENTRY_THR 且今日分值 > 昨日  → 今日收盘买入
    顶部  昨日 ≤ 100 且今日 > 100                → 首次突破
    入场后 COOL_TRADING 个交易日内不再重复入场。

    reso 传入时（由 resonance(panels) 得到），会额外给底部信号打上
    「当日还有几个板块也在 ≤0」的横截面共振标签 reso / resonant。

    返回 (bottom_rows, top_rows)。
    """
    ...
    mv = (mg.reindex(dates, method="ffill").to_numpy(dtype=float)
    ...
    for i in range(1, n - H):
        ...
        if i - last_t >= COOL_TRADING and s[i - 1] <= 100.0 and s[i] > 100.0:
            last_t = i
            has60 = i + H60 <= n - 1
            seg60 = c[i + 1:i + H60 + 1] if has60 else None
            r60 = round(float(seg60[-1] / c0 - 1) * 100, 2) if has60 else None
            ru60 = round(float(seg60.max() / c0 - 1) * 100, 2) if has60 else None
            m0 = float(mv[i]) if (mv is not None and not np.isnan(mv[i])) else None
            top.append({
                "date": str(dates[i].date()),
                "prev": round(float(s[i - 1]), 1),
                "score": round(float(s[i]), 1),
                "ret": round(ret * 100, 2),
                "ret60": r60,
                "risk": round(run * 100, 2),
                "run60": ru60,
                "ok": bool(ret < 0 and run <= TOL),
                "ok60": (bool(seg60[-1] / c0 - 1 < 0 and seg60.max() / c0 - 1 <= TOL)
                         if has60 else None),
                "notrap": False,
                "mg": round(m0, 1) if m0 is not None else None,
                "frenzy": bool(m0 is not None and m0 >= MG_PCT_MIN),
                "kind": "top",
            })
            top[-1]["grade"] = grade(top[-1])
    return bot, top


def baseline(p: pd.DataFrame, kind: str) -> float:
    close = p["close"]
    ok = []
    for i in range(len(close) - H):
        ...
        ret = seg.iloc[-1] / c0 - 1
        if kind == "bottom":
            ok.append(ret > 0 and seg.min() / c0 - 1 >= -TOL)
        else:
            ok.append(ret < 0 and seg.max() / c0 - 1 <= TOL)
...
def summary(panels: dict[str, pd.DataFrame]) -> dict:
    tot = {"b_n": 0, "b_ok": 0, "b_nt": 0, "t_n": 0, "t_ok": 0,
           ...
           "f_n": 0, "f_n60": 0, "f_dn60": 0, "t_n60": 0, "t_dn60": 0,
           **{f"gb{g}_{k}": 0 for g in "ABC" for k in ("n", "ok", "n60", "win60")},
           **{f"gt{g}_{k}": 0 for g in "ABC" for k in ("n", "ok", "n60", "dn60")}}
    per = {}
    ...
    for b, p in panels.items():
        ...
        tok = sum(1 for e in top if e["ok"])
        ...
        # 顶部：杠杆挤满确认层
        fr = [e for e in top if e["frenzy"]]
        fr60 = [e for e in fr if e["ret60"] is not None]
        tot["f_n"] += len(fr)
        tot["f_n60"] += len(fr60)
        tot["f_dn60"] += sum(1 for e in fr60 if e["ret60"] < 0)
        t60 = [e for e in top if e["ret60"] is not None]
        tot["t_n60"] += len(t60)
        tot["t_dn60"] += sum(1 for e in t60 if e["ret60"] < 0)
        # 置信等级统计
        for g in ("A", "B", "C"):
            ...
            gt = [e for e in top if e["grade"] == g]
            gt60 = [e for e in gt if e["ret60"] is not None]
            tot[f"gt{g}_n"] += len(gt)
            tot[f"gt{g}_ok"] += sum(1 for e in gt if e["ok"])
            tot[f"gt{g}_n60"] += len(gt60)
            tot[f"gt{g}_dn60"] += sum(1 for e in gt60 if e["ret60"] < 0)
        per[b] = {
            "bottom": {"n": len(bot), "ok": bok, "nt": bnt},
            "top": {"n": len(top), "ok": tok},
            ...
            "base_bottom": baseline(p, "bottom"),
            "base_top": baseline(p, "top"),
        }
    return {"total": tot, "per": per, "entry_thr": ENTRY_THR,
            "reso_min": RESONANCE_MIN, "bna_min": BNA_PCT_MIN, "mg_min": MG_PCT_MIN,
            "mg_latest": (None if mg is None or len(mg) == 0
                          else round(float(mg.dropna().iloc[-1]), 1)),
